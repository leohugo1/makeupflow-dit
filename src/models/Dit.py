...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context):
        ...
        k, v = map(
            lambda t: t.reshape(B, -1, self.num_heads, C // self.num_heads).transpose(1, 2),
            kv
        )
...
class Block(nn.Module):
    ...
    def forward(self, x, c, t_emb):
        ...
        x_mod = self.modulate(self.norm1(x), shift_msa, scale_msa)
        qkv   = self.to_qkv(x_mod).chunk(3, dim=-1)
        q, k, v = map(
            lambda t: t.reshape(B, N, self.num_heads, C // self.num_heads).transpose(1, 2),
            qkv
        )
        attn_out = F.scaled_dot_product_attention(q, k, v)
        attn_out = attn_out.transpose(1, 2).reshape(B, N, C)
        x = x + gate_msa.unsqueeze(1) * self.attn_out(attn_out)
...
class DiT(nn.Module):
    def __init__(
        self, input_size=64, patch_size=2, in_channels=4,
        hidden_size=768, depth=12, num_heads=12, mlp_ratio=4, style_dim=512,
        ...
        self.num_patchs_side = input_size // patch_size 
    ...
    def timestep_embedding(self, timesteps):
        args = timesteps[:, None].float() * self.t_freqs[None, :]
        return torch.cat([torch.sin(args), torch.cos(args)], dim=-1)

    def unpatchify(self, x):
        p    = self.patch_size
        h = w = self.num_patchs_side 
    ...
    def forward(self, x, t, style_vector):
        ...
        t_emb     = self.t_embedder(self.timestep_embedding(t))
        style_ctx = self.style_proj(style_vector).unsqueeze(1)

        for block in self.blocks:
            if self.use_gradient_checkpointing and self.training:
                x = checkpoint(block, x, style_ctx, t_emb, use_reentrant=False)
            else:
